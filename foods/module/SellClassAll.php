...
/*
	[插件名称] 供应信息分类列表 - 模版标签{#modSellClassAll#}
	[适用范围] 所有页面
	[文 件 名] SellClassAll.php
	[更新时间] 2006/9/18
*/


function SellClassAll(){

		global $msql,$tbl_info_cat,$CatchOpen;
		global $NowMenuid,$MenuInfo,$strMore;

		$PSET=PlusSet("modSellClassAll");
		...
		$msql->query("select * from $tbl_info_cat $scl order by $ord $sc");
		$i=0;
		...
		for($t=0;$t<$TotalCat;$t++){
				if($ArrayPid[$t]==0){
					...
					if($CatchOpen=="1" && file_exists(ROOTPATH."sell/class/".$ArrayCatid[$t].".html")){
						$u=ROOTPATH."sell/class/";
					}else{
						$u=ROOTPATH."sell/class/?";
					}
					...
					$SecondMenu=SellClassSmall("sell",$cutword,$target,$ArrayCatid[$t],$ArrayPid,$TotalCat,$ArrayCatid,$ArrayCat,$ArrayCatpath,$ArrayCatNums);
...
function SellClassSmall($fold,$cutword,$target,$pid,$ArrayPid,$TotalCat,$ArrayCatid,$ArrayCat,$ArrayCatpath,$ArrayCatNums){
