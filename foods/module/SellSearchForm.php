...
/*
	[插件名称] 供应信息组合搜索 - 替换模版的{#modSellSearchForm#}
	[适用范围] 全站
	[文 件 名] SellSearchForm.php
	[更新时间] 2006/9/21
*/


function SellSearchForm(){

	global $msql,$fsql,$tsql,$charset,$PagesInfo,$lanZoneSel,$lanCitySel;
	global $tbl_info_cat,$tbl_member_zone,$tbl_infotype,$MenuInfo;


	$PSET=PlusSet("modSellSearchForm");
	
	$tempname=PlusDef($PSET["tempname"],"tpl_infosearchform.htm");
	...
	//行业分类
	$fsql->query("select * from $tbl_info_cat where pid='0' order by xuhao");
	...
	//企业经营模式
	$fsql->query("select * from $tbl_infotype");
	while($fsql->next_record()){
		$lid=$fsql->f('id');
		$typename=$fsql->f('infotype');
		if($infotype==$lid){
			$infotypelist.="<option value='".$lid."' selected>".$typename."</option>";
		}else{
			$infotypelist.="<option value='".$lid."'>".$typename."</option>";
		}
