...
/*
	[插件名称] 餐馆分类列表 - 模版标签{#modCompanyClassAll#}
	[适用范围] 所有页面
	[文 件 名] CompanyClassAll.php
	[更新时间] 2006/9/17
*/


function CompanyClassAll(){

		global $msql,$tbl_member_cat,$CatchOpen;
		global $NowMenuid,$MenuInfo,$strMore;

		$PSET=PlusSet("modCompanyClassAll");
		...
		$msql->query("select * from $tbl_member_cat $scl order by $ord $sc");
		$i=0;
		...
		for($t=0;$t<$TotalCat;$t++){
				if($ArrayPid[$t]==0){
					...
					if($CatchOpen=="1" && file_exists(ROOTPATH."company/class/".$ArrayCatid[$t].".html")){
						$u=ROOTPATH."company/class/";
					}else{
						$u=ROOTPATH."company/class/?";
					}
					...
					$SecondMenu=CompanyClassSmall("company",$cutword,$target,$ArrayCatid[$t],$ArrayPid,$TotalCat,$ArrayCatid,$ArrayCat,$ArrayCatpath,$ArrayCatNums);
...
function CompanyClassSmall($fold,$cutword,$target,$pid,$ArrayPid,$TotalCat,$ArrayCatid,$ArrayCat,$ArrayCatpath,$ArrayCatNums){
