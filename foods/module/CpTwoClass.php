...
/*
	[插件名称] 图文二级分类 - 模版标签{#modCpTwoClass#}
	[适用范围] 所有页面
	[文 件 名] CpTwoClass.php
	[更新时间] 2006/7/31
*/
...
function CpTwoClass(){
		...
		global $msql,$tbl_cp_cat,$CatchOpen,$strCpClass,$MenuInfo;
		global $NowMenuid;

		$PSET=PlusSet("modCpTwoClass");
		...
		for($t=0;$t<$TotalCat;$t++){
				if($ArrayPid[$t]==$catid){
					...
					$SecondMenu=CpSmallClass($fold,$hangnums,$target,$ArrayCatid[$t],$ArrayPid,$TotalCat,$ArrayCatid,$ArrayCat,$ArrayCatpath,$ArrayCatNums);
...
function CpSmallClass($fold,$hangnums,$target,$pid,$ArrayPid,$TotalCat,$ArrayCatid,$ArrayCat,$ArrayCatpath,$ArrayCatNums){
