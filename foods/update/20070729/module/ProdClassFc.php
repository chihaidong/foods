...
/*
	[插件名称] 产品下一级分类 - 替换模版标签{#modProdClassFc#}
	[适用范围] 检索页(显示当前类别的下一级类别)
	[文 件 名] ProdClassFc.php
	[更新时间] 2006/9/18
*/



function ProdClassFc(){

		global $msql,$tbl_products_cat,$CatchOpen;
		global $NowCatid,$NowMenuid,$MenuInfo;
 
		$PSET=PlusSet("modProdClassFc");
		...
		$msql->query("select * from $tbl_products_cat where $scl order by $ord $sc");
		$i=0;
